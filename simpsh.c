...
void setVar(char name[], char val[]) {
	...
	if (strcmp(name, "PATH") && strcmp(name, "CWD") && strcmp(name, "PS")) { //if not changing one of the three preset variables, the name must be checked to make sure it's valid
		...
		int j = 0;
		strcpy(str, name);
		while (str[j] != 0 && j < sizeof(maxChars)) {
...
void parser(char* tokens[]) {
	...
	else if (tokens[1] != NULL && !strcmp(tokens[1], "="))
